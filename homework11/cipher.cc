...
string Cipher::encrypt(string raw)
{
    ...
    string plain_alpha = smile->cipher_alpha;
    unsigned int pos = 0;
    unsigned int i;
    for (i = 0; i < raw.size(); i++) {
        if (raw[i] == ' ') {
            ...
            pos = find_pos(plain_alpha, raw[i]);
            if (isupper(raw[i])) {
                char cipher_alpha_copy = smile->cipher_alpha[pos];
                retStr.push_back(UPPER_CASE(cipher_alpha_copy));
            }else
...
string Cipher::decrypt(string enc)
{
    ...
    for (i = 0; i < enc.size(); i++) {
        if (enc[i] == ' ') {
            ...
            if (isupper(enc[i])) {
                char plain_alpha_copy = plain_alpha[pos];
                retStr.push_back(UPPER_CASE(plain_alpha_copy));
            }else 
...
unsigned int find_pos(string alpha, char c)
{
    ...
    for (pos = 0; pos < alpha.size(); pos++) {
        /*consider the upper case situation */
        char c_copy = c;
        if (alpha[pos] == LOWER_CASE(c_copy))
            return pos;  
    }
}
