...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    /*iter through every cols of each rows then add the node to the list of each row */
    for (int i = 0; i < rows; i++){
        for (int j = 0; j < cols; j++) { 
            if (adj_mat[i][j]!= 0) {
                adj_node_t *new_Node = create_node(j);
                add_node(myList, i, new_Node);
            }
        }
    }
...
/* Do the list-based BFS, given the source node and the graph's adjacency 
   ...
   return parameters:
       none
 */
/*using the sudo code from professor Jee Whan Choi's slides  */
         ...
         int* color, int* distance, int* parent)
{
    ...
    int u;
    /*keep adding and removeing the value of node to the queue*/
    while (queue != NULL) {
        u = remove_node(&queue);
        for (adj_node_t*node = list[u]; node != NULL; node = node->next) {
            if (color[node->vid] == 0) {
                int v = node->vid;
                distance[v] = distance[u] + 1;
                parent[v] = u;
                color[v] = 1;
                adj_node_t* new_source_node = create_node(v);
                add_node(&queue, 0, new_source_node);
            }
        }
    }
