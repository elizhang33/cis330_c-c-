...
/*A spmv helper function to for the bfs_spmv*/
void spmv(int *dst, int *src, int **source_array, int rows, int cols) {
    int i, j;
    
    /* update the vector for each run*/
    for (i = 0; i < cols; i++) {
        for (j = 0; j < rows; j++) {
            dst[i] += source_array[i][j] * src[j];  
        }
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        spmv(dst, src, mat_trans, rows, cols);
        ...
        /*here is the part I spemt a lot of time to debug because of aliasing
        since i directly assign src = dst. This for loop just make sure updating 
        dst to 0*/
        /*credit from Ellie Yun because she helped me found the Aliasing problem and 
        then I fixed it.
        */
        if (num_zero != rows){
            iter++;
