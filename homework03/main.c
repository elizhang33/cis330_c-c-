...
* Credit: I talked with Jinhu Qi the logic of creating the coo to csr funstion.
* I talked with Eugene Tan about debugging when I faid to have all correct output, 
* but I still didn't fix it until I talked with Xingzhi Wang. Then he helped me debugging 
* with my spmv function. After talked with him, I fixed my problem by editing my for loop 
* and index. 
*/
...
void convert_coo_to_csr(int* row_ind, int* col_ind, double* val, 
    ...
    /*allocate memory*/
    unsigned int* row_ptr = (unsigned int*)malloc(sizeof(unsigned int)*(m+1));
    unsigned int* col_ptr = (unsigned int*)malloc(sizeof(unsigned int)*nnz);
    double* my_csr_vals = (double*)malloc(sizeof(double)*nnz);


    /*set the initial values of row pointer as 0*/
    for (i = 0; i < m; i++) {
        row_ptr[i] = 0;
    }

    /*set row pointer based on the row indices*/
    for (i = 0; i < nnz; i++) {
        row_ptr[row_ind[i]]++;

    }
    
    /*move the row pointer based on the number of nonzeros of each row*/
    for (i = 0, col_num = 0; i < m; i++) {
        curr_ptr = row_ptr[i + 1];
        row_ptr[i] = col_num;
        col_num += curr_ptr; 
    }  
    ...
    /*allocate the pointer to correspoding col and value*/
    for (i = 0; i < nnz; i++) {
      
        row = row_ind[i] - 1;
        dest = row_ptr[row];

        col_ptr[dest] = col_ind[i];
        my_csr_vals[dest] = val[i];
...
{   
    /*reset the result value as 0 first */
    for (int i= 0; i < m; i++) {
      res[i] = 0;
    }
    ...
    /*mutiply the matrix in csr format with the sparse matrix-vectors*/
    for (int i = 0; i < nnz; i++) {
        int col_index = csr_col_ind[i];
        double mat_val = csr_vals[i];
        /*make sure the indexs are changing accordingly */
        loop:
        if (start > csr_row_ptr[row_index]) {
            row_index++;
            goto loop;
        }
        else if (start <= csr_row_ptr[row_index]) {
            start++;
        }

        res[row_index - 1] += mat_val*vector_x[col_index - 1];
        }
    }
